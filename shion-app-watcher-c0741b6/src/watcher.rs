...
impl<R: Runtime> Watcher<R> {
    ...
    pub fn run(self: &Arc<Self>) {
        ...
        let last_running = Arc::new(AtomicBool::new(false));
        let check_timer = Timer::new(Duration::from_secs(10), {
            let last_running = Arc::clone(&last_running);
            let watcher = Arc::clone(&self);
            move || {
                let running = watcher.running.load(Relaxed);
                if !last_running.load(Relaxed) && !running {
                    debug!("check timer resume");
                    watcher.resume();
                }
                last_running.store(running, Relaxed);
            }
        });
